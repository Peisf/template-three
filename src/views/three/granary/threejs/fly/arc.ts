...
function flyArc(lon1, lat1, lon2, lat2) {
  const sphereCoord1 = lon2xyz(R, lon1, lat1)// 经纬度坐标转球面坐标
...
function _3Dto2D(startSphere, endSphere) {
  ...
  const quaternion3DXOY = new THREE.Quaternion().setFromUnitVectors(normal, xoyNormal)
  /* 第一次旋转：飞线起点、结束点从3D空间第一次旋转到XOY平面 */
  const startSphereXOY = startSphere.clone().applyQuaternion(quaternion3DXOY)
  const endSphereXOY = endSphere.clone().applyQuaternion(quaternion3DXOY)
  ...
  const quaternionXOYY = new THREE.Quaternion().setFromUnitVectors(midDir, yDir)

  /* 第二次旋转：使旋转到XOY平面的点再次旋转，实现关于Y轴对称 */
  const startSpherXOYY = startSphereXOY.clone().applyQuaternion(quaternionXOYY)
  const endSphereXOYY = endSphereXOY.clone().applyQuaternion(quaternionXOYY)
   ...
   */
  const quaternionInverse = quaternion3DXOY.clone().invert().multiply(quaternionXOYY.clone().invert())
  return {
    // 返回两次旋转四元数的逆四元数
    quaternion: quaternionInverse,
    // 范围两次旋转后在XOY平面上关于y轴对称的圆弧起点和结束点坐标
    startPoint: startSpherXOYY,
    endPoint: endSphereXOYY
  }
}
...
function arcXOY(startPoint, endPoint) {
  ...
  // 调用圆弧线模型的绘制函数
  const arcline: any = circleLine(flyArcCenter.x, flyArcCenter.y, flyArcR, startAngle, endAngle)
  // const arcline = new THREE.Group();// 不绘制轨迹线，使用THREE.Group替换circleLine()即可
  ...
  // 绘制一段飞线，圆心做坐标原点
  const flyLine: any = createFlyLine(flyArcR, startAngle, startAngle + flyAngle)
